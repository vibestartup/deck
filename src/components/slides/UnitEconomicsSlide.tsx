    ...
    <div className="w-full flex flex-col px-8 py-8">
      ...
      >
        ...
        <div className="text-center mb-16">
          <p className="text-4xl font-bold text-green-400 mb-2">{formatMultiplier(keyMetrics.ltvCacRatio)} LTV/CAC</p>
          <p className="text-lg text-gray-400">Industry benchmark: 3-5x | Payback: {formatMonths(keyMetrics.paybackPeriod)} | Gross margin: {formatPercentage(keyMetrics.grossMargin)}</p>
        </div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-gray-700">
                  <th className="text-left py-4 px-6 text-xl font-bold">Variable</th>
                  <th className="text-center py-4 px-6 text-xl font-bold text-red-400">Pessimistic</th>
                  <th className="text-center py-4 px-6 text-xl font-bold text-yellow-400">Base Case</th>
                  <th className="text-center py-4 px-6 text-xl font-bold text-green-400">Optimistic</th>
                  <th className="text-center py-4 px-6 text-xl font-bold text-blue-400">Y1 Revenue Impact</th>
                </tr>
              </thead>
              <tbody>
                {sensitivities.map((row, index) => (
                  <tr key={index} className="border-b border-gray-800 hover:bg-gray-900/50">
                    <td className="py-4 px-6 font-semibold">{row.variable}</td>
                    <td className="py-4 px-6 text-center text-red-400">{row.pessimistic}</td>
                    <td className="py-4 px-6 text-center text-yellow-400">{row.base}</td>
                    <td className="py-4 px-6 text-center text-green-400">{row.optimistic}</td>
                    <td className="py-4 px-6 text-center text-blue-400 font-bold">{row.impact}</td>
                  </tr>
                ))}
              </tbody>
            </table>
